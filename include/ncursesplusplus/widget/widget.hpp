...
class Widget {
 ...
 public:
  ...
  virtual ~Widget() {
    if (detail::Focus::FocusWidget() == this)
      detail::Focus::Clear();
  }
 ...
 public:
  ...
  /// Post an EnableEvent to this widget, and all descendants.
  /** Will only post a ChildPolishedEvent to the parent if requested. Useful
   *  for enabling a child Widget from a parent's ChildPolishedEvent
  ...
  /// Post a DisableEvent to this widget, and all descendants.
  /** Will only post a ChildPolishedEvent to the parent if requested. Useful
   *  for disabling a child Widget from a parent's ChildPolishedEvent
   *  handler. */
  ...
  /** Animated widgets receiver a TimerEvent every \p period. This Timer
   ...
   *  issues. */
  void enable_animation(AnimationEngine::Period_t period) {
    if (is_animated_)
      return;
    System::animation_engine().RegisterWidget(*this, period);
    is_animated_ = true;
  }
  ...
  void enable_animation(FPS fps) {
    if (is_animated_)
      return;
    System::animation_engine().RegisterWidget(*this, fps);
    is_animated_ = true;
  }

  /// Turn off animation, no more Timer_events will be sent to this Widget.
  /** This Widget will be unregistered from the AnimationEngine held by
   *  System. */
  void disable_animation() {
    if (!is_animated_)
      return;
    System::animation_engine().UnregisterWidget(*this);
    is_animated_ = false;
  }
  ...
  /// Handles EnableEvent objects filtered from other Widgets.
  ...
  /// Handles DisableEvent objects filtered from other Widgets.
  virtual auto disable_event_filter(Widget &receiver) -> bool {
    disabled_filter(receiver);
    return false;
  }

  /// Handles ChildAddedEvent objects filtered from other Widgets.
  ...
  /// Handles ChildRemovedEvent objects filtered from other Widgets.
  ...
  /// Handles ChildPolishedEvent objects filtered from other Widgets.
  ...
  /// Handles MoveEvent objects filtered from other Widgets.
  ...
  /// Handles ResizeEvent objects filtered from other Widgets.
  ...
  /// Handles MousePressEvent objects filtered from other Widgets.
  ...
  /// Handles MouseReleaseEvent objects filtered from other Widgets.
  ...
  /// Handles MouseDoubleClickEvent objects filtered from other Widgets.
  ...
  /// Handles MouseWheelEvent objects filtered from other Widgets.
  ...
  /// Handles MouseMoveEvent objects filtered from other Widgets.
 ...
 public:
  /// Should only be used by MoveEvent send() function.
  void set_top_left(Point p) { top_left_position_ = p; }

  /// Should only be used by ResizeEvent send() function.

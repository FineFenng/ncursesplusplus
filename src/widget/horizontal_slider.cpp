...
auto Horizontal_slider::mouse_wheel_event(Mouse const &m) -> bool {
  switch (m.button) {
    case Mouse::Button::ScrollUp: scrolled_up();
      break;
    case Mouse::Button::ScrollDown: scrolled_down();
      break;
    default: break;
  }
...
auto Horizontal_slider::key_press_event(Key k) -> bool {
  ...
  switch (k) {
    case Key::Arrow_right:this->set_percent(position_to_percent(current_position + 1));
      break;
    case Key::Arrow_left:
